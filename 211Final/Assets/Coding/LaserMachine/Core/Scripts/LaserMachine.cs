            ...
            foreach (LaserElement element in elementsList)
            {
                ...
                if (m_active)
                {
                    ...
                    if (m_currentProperties.m_physicsType == LaserProperties.PhysicsType.Physics3D)
                    {
                        ...
                        else if (hitInfo3D.collider && hitInfo3D.collider.tag == "PlayerCharacter")
                        {
                            GameObject.FindGameObjectWithTag("PlayerCharacter").GetComponent<GameController>().playerDeath();
                        }
                        else
                        {
